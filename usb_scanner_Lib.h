...
#define BARCODE_BUF_LEN 128
static char barcode_buffer[BARCODE_BUF_LEN];
static int barcode_index = 0;
volatile uint8_t barcode_read = 0;
...
typedef void (*barcode_callback_t)(const char *barcode, int length);
static barcode_callback_t barcode_callback = NULL;

void set_barcode_callback(barcode_callback_t cb) {
    barcode_callback = cb;
}
...
static inline void hid_keyboard_print_char(unsigned int key_char) {
  if (!!key_char) {
    ...
    if (KEYBOARD_ENTER_MAIN_CHAR == key_char) {
      ...
      // barcode_read = 1;
      if (barcode_callback) {
          barcode_callback(barcode_buffer, barcode_index);  // Call with length
      }else{
        printf("%s\n\r",barcode_buffer);
      }
      barcode_index = 0;  
...
//     // Ignore empty reports (key released)
//     bool is_empty = true;
//     for (int i = 0; i < length; i++) {
//         if (data[i] != 0x00) {
//             is_empty = false;
...
//     // HID boot keyboard report structure
//     typedef struct {
//         uint8_t modifiers;
//         uint8_t reserved;
//         uint8_t keycode[6];
...
//         char ascii = 0;
//         if (report->modifiers & (HID_KEYBOARD_MODIFIER_LEFT_SHIFT | HID_KEYBOARD_MODIFIER_RIGHT_SHIFT)) {
//             ascii = hid_scancode2ascii[keycode].shift;
//         } else {
//             ascii = hid_scancode2ascii[keycode].normal;
//         }

//         if (ascii) {
//             if (ascii == '\n' || ascii == '\r') {
//                 barcode_buffer[barcode_index] = '\0'; // Null-terminate
//                 printf("Scanned Barcode: %s\n", barcode_buffer);
//                 barcode_index = 0;
//             } else if (barcode_index < BARCODE_BUF_LEN - 1) {
//                 barcode_buffer[barcode_index++] = ascii;
//             }
//         }
//     }
// }
...
void hid_host_device_event(hid_host_device_handle_t hid_device_handle,
  ...
  case ESP_ERR_INVALID_STATE:
    ESP_LOGI(TAG, "HID Device, protocol '%s' INVALID_STATE",
             hid_proto_name_str[dev_params.proto]);
    // Serial.println("HID Device, protocol '");
    // Serial.print(hid_proto_name_str[dev_params.proto]);
